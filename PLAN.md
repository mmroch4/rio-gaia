...
| 2026-03-21 | #9 Security Hardening — Phase 2 (Step 9.6) | — | Initial `express-rate-limit` replaced with custom Medusa-native middleware (async `(MedusaRequest, MedusaResponse, MedusaNextFunction)` signature) due to `wrapHandler` compatibility issues. Auth: 10 req/15min (shared limiter). Quotes: 5 req/hour. Contact: 3 req/hour. In-memory store with periodic cleanup. Storefront server actions (`quotes.ts`, `contact.ts`) catch 429 server-side and return structured `{ success, rateLimited, message }` results — required because Next.js strips `FetchError.status` across the server→client boundary. Contact form shows amber banner, quote creation shows toast, auth shows inline error message. Step 9.7 (CAPTCHA) deferred. | `docs/backend.md`, `docs/api-reference.md`, `docs/architecture.md`, `docs/storefront.md`, `CLAUDE.md`, `PLAN.md` |
