...
import { ModuleCompanyAddress } from "@/types/company"
...
const EditCompanyAddress: React.FC<EditCompanyAddressProps> = ({
  region,
  address,
  companyId,
  isAdmin,
  ...
  const [formState, formAction] = useActionState(updateCompanyAddress, {
    success: false,
    error: null,
  ...
  const removeAddress = async () => {
    setRemoving(true)
    await deleteCompanyAddress(companyId, address.id)
    setRemoving(false)
  }
  ...
  return (
    <>
      <div
        ...
        data-testid="company-address-container"
      >
      ...
      <Modal
        isOpen={state}
        close={close}
        data-testid="edit-company-address-modal"
      >
        <Modal.Title>
          <Heading className="mb-2">Editar morada de envio</Heading>
        </Modal.Title>
